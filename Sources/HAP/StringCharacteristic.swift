...
/// A read request for a String characteristic.
public struct StringCharacteristicReadRequest {
    ...
    public var characteristic: StringCharacteristic
...
/// A write request for a String characteristic.
public struct StringCharacteristicWriteRequest {
...
/// A subscription (or unsubscription) request for a String characteristic.
public struct StringCharacteristicSubscriptionRequest {
    /// Transport type over which the request has been received.
    public var transportType: TransportType
...
public struct StringCharacteristic {
    ...
    /// Maximum UTF-8 string length in bytes (excluding the null terminator).
    public var maxLength: UInt16

    // MARK: Callbacks

    /// Handles read requests. Must not block.
    public var handleRead: ((AccessoryServer, StringCharacteristicReadRequest) throws -> String)?

    /// Handles write requests. Must not block. Value is pre-validated against ``maxLength``.
    public var handleWrite: ((AccessoryServer, StringCharacteristicWriteRequest, String) throws -> Void)?

    /// Handles subscribe requests. Must not block.
    public var handleSubscribe: ((AccessoryServer, StringCharacteristicSubscriptionRequest) -> Void)?

    /// Handles unsubscribe requests. Must not block.
    public var handleUnsubscribe: ((AccessoryServer, StringCharacteristicSubscriptionRequest) -> Void)?
}
