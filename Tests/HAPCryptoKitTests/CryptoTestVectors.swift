// Test vectors extracted from HomeKitADK Tests/HAPCryptoTest.c,
// matching HAP Specification R2, Section 5.5.2 SRP Test Vectors and the
// referenced RFC test vectors (RFC 8032, RFC 7748, RFC 7539, HKDF-SHA-512).

enum CryptoTestVectors {

    static let chacha20Poly1305Aad: [UInt8] = [
        0x50, 0x51, 0x52, 0x53, 0xC0, 0xC1, 0xC2, 0xC3, 0xC4, 0xC5, 0xC6, 0xC7,
    ]

    static let chacha20Poly1305Ct: [UInt8] = [
        0xD3, 0x1A, 0x8D, 0x34, 0x64, 0x8E, 0x60, 0xDB, 0x7B, 0x86, 0xAF, 0xBC,
        0x53, 0xEF, 0x7E, 0xC2, 0xA4, 0xAD, 0xED, 0x51, 0x29, 0x6E, 0x08, 0xFE,
        0xA9, 0xE2, 0xB5, 0xA7, 0x36, 0xEE, 0x62, 0xD6, 0x3D, 0xBE, 0xA4, 0x5E,
        0x8C, 0xA9, 0x67, 0x12, 0x82, 0xFA, 0xFB, 0x69, 0xDA, 0x92, 0x72, 0x8B,
        0x1A, 0x71, 0xDE, 0x0A, 0x9E, 0x06, 0x0B, 0x29, 0x05, 0xD6, 0xA5, 0xB6,
        0x7E, 0xCD, 0x3B, 0x36, 0x92, 0xDD, 0xBD, 0x7F, 0x2D, 0x77, 0x8B, 0x8C,
        0x98, 0x03, 0xAE, 0xE3, 0x28, 0x09, 0x1B, 0x58, 0xFA, 0xB3, 0x24, 0xE4,
        0xFA, 0xD6, 0x75, 0x94, 0x55, 0x85, 0x80, 0x8B, 0x48, 0x31, 0xD7, 0xBC,
        0x3F, 0xF4, 0xDE, 0xF0, 0x8E, 0x4B, 0x7A, 0x9D, 0xE5, 0x76, 0xD2, 0x65,
        0x86, 0xCE, 0xC6, 0x4B, 0x61, 0x16,
    ]

    static let chacha20Poly1305Key: [UInt8] = [
        0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B,
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97,
        0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F,
    ]

    static let chacha20Poly1305Nonce: [UInt8] = [
        0x07, 0x00, 0x00, 0x00, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47,
    ]

    static let chacha20Poly1305Pt = "Ladies and Gentlemen of the class of '99: If I could offer you only one tip for the future, sunscreen would be it."

    static let chacha20Poly1305Tag: [UInt8] = [
        0x1A, 0xE1, 0x0B, 0x59, 0x4F, 0x09, 0xE2, 0x6A, 0x7E, 0x90, 0x2E, 0xCB,
        0xD0, 0x60, 0x06, 0x91,
    ]

    static let ed25519M: [UInt8] = [
        0x72,
    ]

    static let ed25519Pk: [UInt8] = [
        0x3D, 0x40, 0x17, 0xC3, 0xE8, 0x43, 0x89, 0x5A, 0x92, 0xB7, 0x0A, 0xA7,
        0x4D, 0x1B, 0x7E, 0xBC, 0x9C, 0x98, 0x2C, 0xCF, 0x2E, 0xC4, 0x96, 0x8C,
        0xC0, 0xCD, 0x55, 0xF1, 0x2A, 0xF4, 0x66, 0x0C,
    ]

    static let ed25519Sig: [UInt8] = [
        0x92, 0xA0, 0x09, 0xA9, 0xF0, 0xD4, 0xCA, 0xB8, 0x72, 0x0E, 0x82, 0x0B,
        0x5F, 0x64, 0x25, 0x40, 0xA2, 0xB2, 0x7B, 0x54, 0x16, 0x50, 0x3F, 0x8F,
        0xB3, 0x76, 0x22, 0x23, 0xEB, 0xDB, 0x69, 0xDA, 0x08, 0x5A, 0xC1, 0xE4,
        0x3E, 0x15, 0x99, 0x6E, 0x45, 0x8F, 0x36, 0x13, 0xD0, 0xF1, 0x1D, 0x8C,
        0x38, 0x7B, 0x2E, 0xAE, 0xB4, 0x30, 0x2A, 0xEE, 0xB0, 0x0D, 0x29, 0x16,
        0x12, 0xBB, 0x0C, 0x00,
    ]

    static let ed25519Sk: [UInt8] = [
        0x4C, 0xCD, 0x08, 0x9B, 0x28, 0xFF, 0x96, 0xDA, 0x9D, 0xB6, 0xC3, 0x46,
        0xEC, 0x11, 0x4E, 0x0F, 0x5B, 0x8A, 0x31, 0x9F, 0x35, 0xAB, 0xA6, 0x24,
        0xDA, 0x8C, 0xF6, 0xED, 0x4F, 0xB8, 0xA6, 0xFB,
    ]

    static let hkdfIKM: [UInt8] = [
        0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B,
        0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B,
    ]

    static let hkdfOKM: [UInt8] = [
        0x83, 0x23, 0x90, 0x08, 0x6C, 0xDA, 0x71, 0xFB, 0x47, 0x62, 0x5B, 0xB5,
        0xCE, 0xB1, 0x68, 0xE4, 0xC8, 0xE2, 0x6A, 0x1A, 0x16, 0xED, 0x34, 0xD9,
        0xFC, 0x7F, 0xE9, 0x2C, 0x14, 0x81, 0x57, 0x93, 0x38, 0xDA, 0x36, 0x2C,
        0xB8, 0xD9, 0xF9, 0x25, 0xD7, 0xCB,
    ]

    static let hkdfInfo: [UInt8] = [
        0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9,
    ]

    static let hkdfSalt: [UInt8] = [
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B,
        0x0C,
    ]

    static let rfc7748AlicePkey: [UInt8] = [
        0x85, 0x20, 0xF0, 0x09, 0x89, 0x30, 0xA7, 0x54, 0x74, 0x8B, 0x7D, 0xDC,
        0xB4, 0x3E, 0xF7, 0x5A, 0x0D, 0xBF, 0x3A, 0x0D, 0x26, 0x38, 0x1A, 0xF4,
        0xEB, 0xA4, 0xA9, 0x8E, 0xAA, 0x9B, 0x4E, 0x6A,
    ]

    static let rfc7748AliceSkey: [UInt8] = [
        0x77, 0x07, 0x6D, 0x0A, 0x73, 0x18, 0xA5, 0x7D, 0x3C, 0x16, 0xC1, 0x72,
        0x51, 0xB2, 0x66, 0x45, 0xDF, 0x4C, 0x2F, 0x87, 0xEB, 0xC0, 0x99, 0x2A,
        0xB1, 0x77, 0xFB, 0xA5, 0x1D, 0xB9, 0x2C, 0x2A,
    ]

    static let rfc7748Csec1: [UInt8] = [
        0x95, 0xCB, 0xDE, 0x94, 0x76, 0xE8, 0x90, 0x7D, 0x7A, 0xAD, 0xE4, 0x5C,
        0xB4, 0xB8, 0x73, 0xF8, 0x8B, 0x59, 0x5A, 0x68, 0x79, 0x9F, 0xA1, 0x52,
        0xE6, 0xF8, 0xF7, 0x64, 0x7A, 0xAC, 0x79, 0x57,
    ]

    static let rfc7748Pkey1: [UInt8] = [
        0xE5, 0x21, 0x0F, 0x12, 0x78, 0x68, 0x11, 0xD3, 0xF4, 0xB7, 0x95, 0x9D,
        0x05, 0x38, 0xAE, 0x2C, 0x31, 0xDB, 0xE7, 0x10, 0x6F, 0xC0, 0x3C, 0x3E,
        0xFC, 0x4C, 0xD5, 0x49, 0xC7, 0x15, 0xA4, 0x93,
    ]

    static let rfc7748Skey1: [UInt8] = [
        0x4B, 0x66, 0xE9, 0xD4, 0xD1, 0xB4, 0x67, 0x3C, 0x5A, 0xD2, 0x26, 0x91,
        0x95, 0x7D, 0x6A, 0xF5, 0xC1, 0x1B, 0x64, 0x21, 0xE0, 0xEA, 0x01, 0xD4,
        0x2C, 0xA4, 0x16, 0x9E, 0x79, 0x18, 0xBA, 0x0D,
    ]

    static let sha512Hash: [UInt8] = [
        0xDD, 0xAF, 0x35, 0xA1, 0x93, 0x61, 0x7A, 0xBA, 0xCC, 0x41, 0x73, 0x49,
        0xAE, 0x20, 0x41, 0x31, 0x12, 0xE6, 0xFA, 0x4E, 0x89, 0xA9, 0x7E, 0xA2,
        0x0A, 0x9E, 0xEE, 0xE6, 0x4B, 0x55, 0xD3, 0x9A, 0x21, 0x92, 0x99, 0x2A,
        0x27, 0x4F, 0xC1, 0xA8, 0x36, 0xBA, 0x3C, 0x23, 0xA3, 0xFE, 0xEB, 0xBD,
        0x45, 0x4D, 0x44, 0x23, 0x64, 0x3C, 0xE8, 0x0E, 0x2A, 0x9A, 0xC9, 0x4F,
        0xA5, 0x4C, 0xA4, 0x9F,
    ]

    static let shaText = "abc"

    static let srpClientPublicKey: [UInt8] = [
        ,
    ]

    static let srpServerPublicKey: [UInt8] = [
        ,
    ]

    static let srpPremasterSecret: [UInt8] = [
        ,
    ]

    static let srpServerPrivateKey: [UInt8] = [
        0xE4, 0x87, 0xCB, 0x59, 0xD3, 0x1A, 0xC5, 0x50, 0x47, 0x1E, 0x81, 0xF0,
        0x0F, 0x69, 0x28, 0xE0, 0x1D, 0xDA, 0x08, 0xE9, 0x74, 0xA0, 0x04, 0xF4,
        0x9E, 0x61, 0xF5, 0xD1, 0x05, 0x28, 0x4D, 0x20,
    ]

    static let srpSessionKey: [UInt8] = [
        0x5C, 0xBC, 0x21, 0x9D, 0xB0, 0x52, 0x13, 0x8E, 0xE1, 0x14, 0x8C, 0x71,
        0xCD, 0x44, 0x98, 0x96, 0x3D, 0x68, 0x25, 0x49, 0xCE, 0x91, 0xCA, 0x24,
        0xF0, 0x98, 0x46, 0x8F, 0x06, 0x01, 0x5B, 0xEB, 0x6A, 0xF2, 0x45, 0xC2,
        0x09, 0x3F, 0x98, 0xC3, 0x65, 0x1B, 0xCA, 0x83, 0xAB, 0x8C, 0xAB, 0x2B,
        0x58, 0x0B, 0xBF, 0x02, 0x18, 0x4F, 0xEF, 0xDF, 0x26, 0x14, 0x2F, 0x73,
        0xDF, 0x95, 0xAC, 0x50,
    ]

    static let srpProofM1: [UInt8] = [
        0x5F, 0x7C, 0x14, 0xAB, 0x57, 0xED, 0x0E, 0x94, 0xFD, 0x1D, 0x78, 0xC6,
        0xB4, 0xDD, 0x09, 0xED, 0x7E, 0x34, 0x0B, 0x7E, 0x05, 0xD4, 0x19, 0xA9,
        0xFD, 0x76, 0x0F, 0x6B, 0x35, 0xE5, 0x23, 0xD1, 0x31, 0x07, 0x77, 0xA1,
        0xAE, 0x1D, 0x28, 0x26, 0xF5, 0x96, 0xF3, 0xA8, 0x51, 0x16, 0xCC, 0x45,
        0x7C, 0x7C, 0x96, 0x4D, 0x4F, 0x44, 0xDE, 0xD5, 0x55, 0x9D, 0xA8, 0x18,
        0xC8, 0x8B, 0x61, 0x7F,
    ]

    static let srpProofM2: [UInt8] = [
        0x2F, 0xA0, 0xE8, 0x1F, 0x5C, 0xB7, 0x3B, 0x88, 0xFA, 0x09, 0x64, 0x27,
        0x0F, 0x32, 0x1D, 0xD6, 0x41, 0xF2, 0x22, 0x7A, 0x5D, 0x80, 0x5C, 0x40,
        0xF1, 0xBF, 0xE9, 0x6A, 0xAF, 0x6A, 0x19, 0xFF, 0xCE, 0x8E, 0x23, 0x28,
        0x79, 0x65, 0xA3, 0x9E, 0xAB, 0x9D, 0x5A, 0x02, 0x21, 0x5F, 0x89, 0xE1,
        0x28, 0x17, 0x7E, 0xD2, 0xC4, 0xF1, 0x03, 0xE6, 0x55, 0xA0, 0x45, 0x53,
        0x1B, 0xCB, 0xF7, 0xAD,
    ]

    static let srpPassword = "password123"

    static let srpSalt: [UInt8] = [
        0xBE, 0xB2, 0x53, 0x79, 0xD1, 0xA8, 0x58, 0x1E, 0xB5, 0xA7, 0x27, 0x67,
        0x3A, 0x24, 0x41, 0xEE,
    ]

    static let srpScramblingParameter: [UInt8] = [
        0x03, 0xAE, 0x5F, 0x3C, 0x3F, 0xA9, 0xEF, 0xF1, 0xA5, 0x0D, 0x7D, 0xBB,
        0x8D, 0x2F, 0x60, 0xA1, 0xEA, 0x66, 0xEA, 0x71, 0x2D, 0x50, 0xAE, 0x97,
        0x6E, 0xE3, 0x46, 0x41, 0xA1, 0xCD, 0x0E, 0x51, 0xC4, 0x68, 0x3D, 0xA3,
        0x83, 0xE8, 0x59, 0x5D, 0x6C, 0xB5, 0x6A, 0x15, 0xD5, 0xFB, 0xC7, 0x54,
        0x3E, 0x07, 0xFB, 0xDD, 0xD3, 0x16, 0x21, 0x7E, 0x01, 0xA3, 0x91, 0xA1,
        0x8E, 0xF0, 0x6D, 0xFF,
    ]

    static let srpUsername = "alice"

    static let srpVerifier: [UInt8] = [
        ,
    ]
}
