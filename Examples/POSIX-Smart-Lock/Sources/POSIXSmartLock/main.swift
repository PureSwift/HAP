...
//
// Usage:
//   posix-smart-lock [--setup-code XXX-XX-XXX] [--name <name>] [--storage <directory>]
//
...
let name = option("name") ?? "Front Door"
...
let accessory = SmartLock.makeAccessory(name: name, serialNumber: "0000000001")
try accessory.validate(transport: .ble)

let dataSource = StandardDataSource(application: LockDataSource(name: name))
